...
export type VariablesController = {
  setVariables: (items: VariableItem[], truncated?: boolean) => void;
  clear: () => void;
  toggle: () => void;
};

function formatType(item: VariableItem) {
...
export function createVariablesController(dom: DomRefs): VariablesController {
  let isOpen = false;
  ...
  const setVariables = (items: VariableItem[], truncated = false) => {
    if (!items.length) {
