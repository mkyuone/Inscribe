...
export function createFindReplaceController(
  ...
  let markers: CodeMirrorTextMarker[] = [];
  let invalidRegex = false;

  function getFocusedFindField() {
    const active = document.activeElement;
    if (active === dom.findInput) return dom.findInput;
    if (active === dom.replaceInput) return dom.replaceInput;
    return null;
  }
  ...
  function getRegex(useGlobal: boolean) {
  ...
  function nextIndexFromCursor(reverse: boolean) {
    ...
    if (reverse) {
      for (let i = matches.length - 1; i >= 0; i -= 1) {
        if (matches[i].end < cursorIndex) return i;
      }
      return matches.length - 1;
    }

    for (let i = 0; i < matches.length; i += 1) {
      if (matches[i].start > cursorIndex) return i;
    }
    return 0;
  }
  ...
  function replaceCurrent() {
    if (!matches.length || activeMatchIndex < 0) {
      findNext(false);
      if (!matches.length || activeMatchIndex < 0) return;
    }
  ...
  dom.findBtn.addEventListener("click", () => {
    if (isFindOpen() && !isReplaceMode()) closeFind();
    else openFind();
  });
