...
import type { ImportedTabDoc } from "./tabs.js";
import {
  WorkspaceBundle,
  arrayBufferToBase64,
  base64ToUint8Array,
...
async function readWorkspaceUpload(file: File): Promise<ImportedTabDoc> {
  const path = normalizeWorkspacePath((file as File & { webkitRelativePath?: string }).webkitRelativePath || file.name);
...
export function createFileController(
  ...
  let pendingOpenMode: "singleText" | "workspaceFiles" = "workspaceFiles";

  const setFilePickerMode = (mode: "singleText" | "workspaceFiles") => {
  ...
  async function importSelectedFiles(
    ...
    opts: { allowBundle: boolean; successTitle: string; successIcon: string; emptyTitle: string }
  ) {
  ...
  async function importSingleTextFile(input: HTMLInputElement) {
  ...
  dom.directoryInput.addEventListener("change", async (e) => {
    await importSelectedFiles(e.target as HTMLInputElement, {
      allowBundle: false,
  ...
  function saveFile() {
    ...
    if (!isWorkspaceEnabled()) {
      ...
      if (active.encoding === "utf8") {
        ...
        downloadBlob(
          new Blob([base64ToUint8Array(active.content)], {
    ...
    const filename = `${getBasename(active?.name || "workspace").replace(/\.[^.]+$/, "") || "workspace"}.inscribeproj`;
  ...
  function downloadActiveFile() {
    ...
    if (active.encoding === "utf8") {
      ...
      downloadBlob(
        new Blob([base64ToUint8Array(active.content)], { type: active.mime || "application/octet-stream" }),
        getBasename(active.name)
      );
    }
