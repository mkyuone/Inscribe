...
export function normalizeWorkspacePath(name) {
...
export function getFileIcon(path, mime = "") {
...
export function dataUrlFromBase64(base64, mime) {
    return `data:${mime || "application/octet-stream"};base64,${base64}`;
}
export function parseWorkspaceBundle(text) {
    try {
        const parsed = JSON.parse(text);
        if (parsed.format !== "inscribe-workspace-v1" || !Array.isArray(parsed.files))
            return null;
