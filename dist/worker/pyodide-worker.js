...
function toWorkspaceSnapshot(message) {
...
function getFs() {
    var _a;
    return (_a = pyodide === null || pyodide === void 0 ? void 0 : pyodide.FS) !== null && _a !== void 0 ? _a : null;
}
...
function snapshotWorkspaceSync(source) {
    const FS = getFs();
    if (!FS) {
        return {
    ...
    const mimeByPath = new Map(source.files.map((file) => [normalizeWorkspacePath(file.path), file.mime || ""]));
    const originalPaths = new Set(source.files.map((file) => normalizeWorkspacePath(file.path)).filter(Boolean));
    const currentPaths = new Set();
    const results = [];
    const walk = (dir) => {
        for (const child of FS.readdir(dir)) {
            if (child === "." || child === ".." || child === "__pycache__")
                continue;
            ...
            const bytes = FS.readFile(fullPath, { encoding: "binary" });
            try {
                results.push({
                    path: relPath,
...
async function prepareWorkspaceEnvironment(activePath) {
    const workspaceRootLiteral = JSON.stringify(WORKSPACE_ROOT);
    const normalizedActivePath = normalizeWorkspacePath(activePath);
    const activeDir = normalizedActivePath && normalizedActivePath.includes("/")
        ? `${WORKSPACE_ROOT}/${normalizedActivePath.slice(0, normalizedActivePath.lastIndexOf("/"))}`
        : WORKSPACE_ROOT;
            ...
            try {
                ...
                if (mainGlobals) {
                    ...
                    if (message.mode === "all" && activeFsPath.endsWith(".py")) {
                        const runCode = `__inscribe_path = ${JSON.stringify(activeFsPath)}\n` +
